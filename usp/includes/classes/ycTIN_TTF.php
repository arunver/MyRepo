...
class ycTIN_TTF {
	...
	public function generateFontImage($fontPath='' , $imageName)
	{
		$objFont = (object) $objFont;
		$imageName = str_replace('TTF' , 'png' , strtoupper($imageName));

		$fontImage = __FONTORIGINAL__.'/'.$imageName;
		$this->open($fontPath);	//Open Font File
		$fontDetail = $this->getNameTable();		//Get Font Detail

		$fontText 	= $fontDetail['1::0::0'][1];   	//Get Font Style
		
		exec(IMAGEMAGICPATH." -transparent white -trim -font $fontPath -size 250x40 label:$fontText $fontImage" , $result); 
