...
class MySqlDriver implements DB
{
	...
	function db_fetch_assoc_array($result){		
		$dataArray = array();
		while($data = $this->getResultArray($result)){
			$dataArray[] = $data;
		}
		return $dataArray;
	}
	
	function db_fetch_number_array($result){
		$dataArray = array();
	...
	function getResultArray($DataSet) 
	{
    ...
    function singleValueAssoc($tbl,$con){
		$sql="select * from ".$tbl." where ".$con;		
        $result=mysql_query($sql) or die(mysql_error());
...
//	echo "<br>".$str."<br>";
		...
		/*if($q[0]){
			$this->logSuccessFail("1",$str);
		}else{
			$this->logSuccessFail("0",$str);
		}*/
